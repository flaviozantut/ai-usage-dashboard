...
/**
 * Cliente compartilhado da Admin API do Cursor — usado pelo hook e pelo backfill.
 * Dois endpoints:
 *   - filtered-usage-events : tokens EXATOS por evento  → eventos "message"
 *   - daily-usage-data      : produtividade diária       → eventos "productivity"
 *                             (linhas aceitas/rejeitadas, tabs, applies → accept rate)
 */
...
/** Tokens exatos → eventos "message". Paginado. `userEmail` opcional filtra ao dono. */
...
/** Produtividade diária → eventos "productivity" (accept rate de linhas e de tabs). */
